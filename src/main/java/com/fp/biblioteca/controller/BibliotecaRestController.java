...
public class BibliotecaRestController {
	...
	@Operation(summary = "Get all books containing q word in title or author")
	@GetMapping(value = "/libros/search")
	public List<LibroDto> search(@RequestParam String q) {
		List<LibroDto> libros = bibliotecaService.searchLibros(q);
		return libros;
	}
	...
	public ResponseEntity<?> updateBook(@PathVariable("libroId") Long libroId, @Valid @RequestBody LibroDto libroDto) {
		Optional<LibroDto> libroDtoUpdated = bibliotecaService.update(libroId, libroDto);
	...
	public ResponseEntity<?> delete(@PathVariable("libroId") Long libroId) {
		boolean deleted = bibliotecaService.delete(libroId);
		if (deleted) {
